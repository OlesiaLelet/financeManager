  ...
  return (
    ...
    >
      {transactions?.map((item: SpendingsOrIncome) => (
        <Box
          sx={{ boxShadow: "rgba(0, 0, 0, 0.24) 0px 3px 8px", borderRadius: 5 }}
          key={item.id}
        >
          <Typography
            variant="body1"
          ...
          >
            {`${item.date?.toDate()?.getDate()} ${item.date
              ?.toDate()
          ...
          >
            ...
            <ListItemText
              className={item.type === "income" ? "income" : "spending"}
              sx={{ alignSelf: "flex-start", paddingLeft: 18 }}
            >
