...
import { Select, MenuItem, TextField, FormControl, InputLabel, Box, Button } from '@mui/material'
...
export default function CreateSpendings() {
  ...
  const categoriesOfSpendings = useAppSelector(state => state.financeManager.categoryOfSpendings);
  const categoryOfAccount = useAppSelector(state => state.financeManager.categoryOfAccount);
  const amountOfTransactions = useAppSelector(state => state.financeManager.transactions);
  ...
  const handlerSubmit = (event: any) => {
    ...
    if(!select.catOfSpendings) {
      ...
      const iconOfSpend = categoriesOfSpendings.find((item)=> item.name===select.catOfSpendings)?.icon ?? DisabledByDefaultIcon;
     ...
     alert("Spendings succesfully added");
  ...
  return (
    <form style={{display: "flex", flexDirection:"column", gap: 15, alignItems:'center'}}>

     <TextField variant='standard' label='Amount' type='number' value={sum!==0?sum:""} onChange={handlerSum} sx={{width: 550}} name='quantity' error={errors.quantity} required></TextField>
     

      <TextField variant='standard' label='Description (optional)' value={description} onChange={handlerInput} sx={{width: 550}} name="description" maxRows={3} type='text' error= {errors.description} ></TextField>
    ...
    <Button sx={{width: 550}} variant='contained' onClick={handlerSubmit}>Add transaction</Button>
